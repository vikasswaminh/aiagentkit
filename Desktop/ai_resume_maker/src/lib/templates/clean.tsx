            ...
            <div className="px-10 py-6 space-y-6 flex-1">
                ...
                {data.basics?.summary && (
                    <section>
                        ...
                        <p className="text-[10pt] text-slate-600 leading-relaxed text-justify">{data.basics.summary}</p>
                    </section>
                )}
                ...
                {data.experience && data.experience.length > 0 && (
                    <section>
                        <h2 className={`text-xs uppercase tracking-widest mb-3 font-black ${c.primary}`}>Professional Experience</h2>
                        <div className="space-y-5">
                            {data.experience.map((exp, i) => (
                                <div key={i}>
                                    <div className="flex justify-between items-baseline mb-1">
                                        <h3 className="font-bold text-slate-900 text-[12pt] leading-none">{exp.role}</h3>
                                        <span className={`text-[9.5pt] font-bold uppercase tracking-tight ${c.accent}`}>{exp.startDate} — {exp.endDate || 'Present'}</span>
                                    </div>
                                    <div className={`text-[10.5pt] font-bold uppercase tracking-wide mb-2 ${c.primary}`}>{exp.company}</div>
                                    <ul className={`list-disc list-outside ml-4 space-y-1 text-slate-600 text-[9.5pt] leading-tight marker:text-slate-300`}>
                        ...
                        <div className="space-y-4">
                            {data.education.map((edu, i) => (
                                <div key={i} className="flex justify-between items-start">
                                    ...
                                    <span className={`text-[9.5pt] font-bold uppercase tracking-tight ${c.accent}`}>{edu.startDate} — {edu.endDate || 'Present'}</span>
                                </div>
                            ))}
                        </div>
                    </section>
                ...
                {/* Skills - Simple tags style */}
                {data.skills && data.skills.length > 0 && (
                    <section>
                        <h2 className={`text-xs uppercase tracking-widest mb-3 font-black ${c.primary}`}>Technical Skills</h2>
                        <div className="flex flex-wrap gap-2 pt-1 font-sans">
                            {data.skills.map((skill, i) => (
                                <span key={i} className="text-[9pt] text-slate-600 font-bold border border-slate-100 px-2 py-0.5 rounded-sm">
                                    {skill}
                                </span>
                            ))}
                        </div>
                ...
                <div className="grid grid-cols-2 gap-10 pt-2 border-t border-slate-50">
                    {/* Achievements */}
                    {data.achievements && data.achievements.length > 0 && (
                        <section>
                            <h2 className={`text-xs uppercase tracking-widest mb-3 font-black ${c.primary}`}>Achievements</h2>
                            <ul className="list-disc list-outside ml-4 space-y-1.5 text-[9pt] text-slate-600 font-medium leading-tight marker:text-slate-200">
                    ...
                    {data.certifications && data.certifications.length > 0 && (
                        <section>
                            <h2 className={`text-xs uppercase tracking-widest mb-3 font-black ${c.primary}`}>Certifications</h2>
                            <ul className="list-disc list-outside ml-4 space-y-1.5 text-[9pt] text-slate-600 font-medium leading-tight marker:text-slate-200">
